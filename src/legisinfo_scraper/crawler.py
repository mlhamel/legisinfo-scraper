...
def scrape_bill(session, bill_number, cache_bill_dir, repo_path, already_downloaded_stages, dry_run=False):
    ...
    if not stages_to_download and not os.path.exists(target_bill_xml_path) and xml_links:
        ...
        if sorted_available:
            ...
            try:
                ...
                if res.status_code == 200:
                    if not dry_run:
                        # Save directly in cache_bill_dir to indicate they are restore drafts
