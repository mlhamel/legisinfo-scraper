...
def report_status(repo_path):
    ...
    print(f"Fetching global bills list from LEGISinfo...")
    try:
        ...
        print(f"Error fetching bills list: {e}")
        sys.exit(1)
    ...
    print("\nSession Scraping Status Report:")
    print("=" * 70)
    print(f"{'Session':<10} | {'Total Bills':<12} | {'Scraped Bills':<14} | {'Status':<12}")
    print("-" * 70)
    ...
    for sess_code in sorted(api_sessions.keys()):
        ...
        print(f"{sess_code:<10} | {total_api_bills:<12} | {scraped_count:<14} | {status:<12}")
